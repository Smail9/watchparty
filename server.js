// server.js — Watch Party + Proxy vidéo (Express + WebSocket, CommonJS)
...
function ensureContentType(req, proxyRes, fallback = 'video/mp4') {
  const current = String(proxyRes.headers['content-type'] || '').toLowerCase();
  if (current && !/octet-stream/.test(current)) return;
...
/* ---------- /video-remote : exemple d’URL fixe ---------- */
...
/* ---------- /proxy?u=<URL> : proxy dynamique (playlist + segments + mp4) ---------- */
...
app.use('/proxy', (req, res, next) => {
  ...
  return createProxyMiddleware({
    ...
    changeOrigin: true,
    secure: false,
    followRedirects: true,                    // suit 302/307/308 côté serveur
...
app.get('/ts2m3u8', (req, res) => {
  ...
  // On passe le segment via le proxy
  const proxiedTs = '/proxy?u=' + encodeURIComponent(tsURL.toString());

  // Mini manifeste HLS (un seul segment). Durée indicative 60s.
  const m3u8 =
`#EXTM3U
#EXT-X-VERSION:3
...
// roomId -> { clients:Set<ws>, state:{ playing, time, updatedAt, src } }
const rooms = new Map();

function getOrCreateRoom(roomId) {
  if (!rooms.has(roomId)) {
    rooms.set(roomId, { clients: new Set(), state: { playing: false, time: 0, updatedAt: Date.now(), src: null } });
  }
